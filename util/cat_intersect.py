...
# =============================================================================
new_zari_id = [x for x in zari_id if x not in list(intersect)]

# Save the filtered catalog
# np.savetxt('zariv2.cat', X=new_zari_id)

#%%
narray = np.array([1,2,3,4,5,6])
np.argwhere(narray == narray[:2])
#%%
...
#%%
df.loc[df['gdr2_id'] == 4176911701735537664]['id']
df.loc[df['gdr2_id'] == 4176911701735537664]['raj2000']

df.loc[df['gdr2_id'] == 3239181321516170752]['id']

#%%

## Transform Zari's catalog to RA/DEC and save as CSV file with GAIA_ID
from astropy.io import fits
from astropy import units as u
from astropy.coordinates import SkyCoord
...
hdul[1].header
cat = hdul[1].data
str_id = cat['source_id'].astype(str)

gal_coord = SkyCoord(cat['l']*u.deg, cat['b']*u.deg, frame = 'galactic')

data_todf = np.transpose([str_id, gal_coord.icrs.ra.deg,gal_coord.icrs.dec.deg])
df = pd.DataFrame(data_todf,
...
df.to_csv('cat_leiden_idstr.csv', index=False)
